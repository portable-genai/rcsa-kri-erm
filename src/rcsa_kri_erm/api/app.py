"""FastAPI application for RCSA, KRI and ERM Operating Copilot (rcsa-kri-erm).
...
``ERM_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``ERM_S2S_TOKEN`` is wrong in the most dangerous direction:
...
_TOKEN_ENV = "ERM_S2S_TOKEN"
_INSECURE_DEMO_ENV = "ERM_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "ERM_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "ERM_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "ERM_S2S_AUDIENCE"
_API_HOST_ENV = "ERM_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``ERM_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``ERM_PROFILE`` must never be allowed to
    ...
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "ERM_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_APP_TITLE = "RCSA, KRI and ERM Operating Copilot"
...
#
# Note what is NOT in this expression: ERM_S2S_TOKEN. A service
...
def _tenant_access_denied(_request: Request, exc: Exception) -> JSONResponse:
    """A cross-tenant read is a 403 on EVERY route, present and future, never a 500.

    Registered on the app rather than caught per route, deliberately. The refusal is raised deep
    in a port adapter, and the alternative shape is a route that forgets the try/except and
    answers 500: an isolation refusal reported as a server fault reads as a bug to fix rather
    than a boundary holding, and it leaks a stack trace where it should leak a status. Today
    ``/v1/triage`` is the only end-user route and it reaches no tenant-scoped port, so nothing
    here can raise it yet; the handler exists so the first ERM route added inherits the right
    answer instead of having to remember it.

    403 rather than 404: the records EXIST and this caller may not have them, and 404 would make
    the partition probeable with a tenant-name generator. The detail is the adapter's own
    message, which names no record and no other tenant's data.
    """
