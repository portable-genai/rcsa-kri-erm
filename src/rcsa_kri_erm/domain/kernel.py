...
def redacted_citations(citations: tuple[Citation, ...]) -> tuple[Citation, ...]:
    ...
    case subject>``, ``control:<the control id>``), so both are text this service did not author,
    wearing a structural-looking name. No sink can tell an engine-built citation from an
    extractor-built one by inspection, so this masks unconditionally: redacting text that carries
    no identifier is a no-op, while deciding per caller is how one caller ends up forgetting.
    """
    return tuple(
        Citation(
...
class AuditEvent:
    ...
    long-retained by design. Eight such call sites shared this type (``triage_service`` and the
    seven ``erm_service._record`` paths), so construction masks every CONTENT field instead: the
    summary, and each citation's locator, title and snippet. Redaction is idempotent, so a caller
    that already redacted loses nothing by the second pass, and a surface added later inherits
