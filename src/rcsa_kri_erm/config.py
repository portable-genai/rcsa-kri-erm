...
One env var (``ERM_PROFILE``) selects the adapter family for every
...
_PROFILE_ENV = "ERM_PROFILE"
_SETTINGS_ENV = "ERM_SETTINGS"
...
class ProfileChoice:
    ...
    The variable is ``ERM_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("ERM_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "control_library": {
        "local": f"{_PKG}.adapters.local.control_library:LocalControlLibraryAdapter",
        "gcp": f"{_PKG}.adapters.gcp.control_library:CloudControlLibraryAdapter",
        "onprem": f"{_PKG}.adapters.onprem.control_library:OnPremControlLibraryAdapter",
    },
    "embeddings": {
        "local": f"{_PKG}.adapters.local.embeddings:LocalEmbeddingsAdapter",
        "gcp": f"{_PKG}.adapters.gcp.embeddings:CloudEmbeddingsAdapter",
        "onprem": f"{_PKG}.adapters.onprem.embeddings:OnPremEmbeddingsAdapter",
    },
    "metric_feed": {
        "local": f"{_PKG}.adapters.local.metric_feed:LocalMetricFeedAdapter",
        "gcp": f"{_PKG}.adapters.gcp.metric_feed:CloudMetricFeedAdapter",
        "onprem": f"{_PKG}.adapters.onprem.metric_feed:OnPremMetricFeedAdapter",
    },
    "theme_feed": {
        "local": f"{_PKG}.adapters.local.theme_feed:LocalThemeFeedAdapter",
        "gcp": f"{_PKG}.adapters.gcp.theme_feed:CloudThemeFeedAdapter",
        "onprem": f"{_PKG}.adapters.onprem.theme_feed:OnPremThemeFeedAdapter",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``ERM_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "ERM_REVIEW_ROUTING"
...
class Container:
    ...
    @cached_property
    def control_library(self) -> ControlLibraryPort:
        adapter = self._bind("control_library")
        assert isinstance(adapter, ControlLibraryPort)
        return adapter

    @cached_property
    def embeddings(self) -> EmbeddingsPort:
        adapter = self._bind("embeddings")
        assert isinstance(adapter, EmbeddingsPort)
        return adapter

    @cached_property
    def metric_feed(self) -> MetricFeedPort:
        adapter = self._bind("metric_feed")
        assert isinstance(adapter, MetricFeedPort)
        return adapter

    @cached_property
    def theme_feed(self) -> ThemeFeedPort:
        adapter = self._bind("theme_feed")
        assert isinstance(adapter, ThemeFeedPort)
        return adapter
