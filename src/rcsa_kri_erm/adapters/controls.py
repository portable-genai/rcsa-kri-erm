...
(``ERM_REVIEW_ROUTING=off``), the container binds :class:`DisabledReviewRouter` instead of the
...
what it returns can say what happened: ``routed``, ``failed``, ``off`` or ``not_required``. The
ERM tools pass the wrapper INTO :class:`~rcsa_kri_erm.domain.erm_service.ErmService` in place of
the bound router, so the domain routes exactly as before and the wrapper sits outside it. A
