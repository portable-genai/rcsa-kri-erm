...
SKILLS: tuple[AgentSkill, ...] = (
    ...
    AgentSkill(
        id="assess_rcsa_control",
        name="RCSA residual-risk assessment",
        description=(
            "Score one control's residual risk from an accepted impact/likelihood/effectiveness "
            "rating with a config-owned matrix, narrate it, and ROUTE the result to human review "
            "when the residual band is consequential (rule R8). The score is pure stdlib code."
        ),
    ),
    AgentSkill(
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://rcsa-kri-erm.{settings.region}.internal.example/a2a"
