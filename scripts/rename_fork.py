...
The catalog id (``Erm1``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
_OLD_PACKAGE = "rcsa_kri_erm"
_OLD_ENV_PREFIX = "ERM_"
_OLD_BARE_PREFIX = "ERM"
_OLD_RESOURCE = "erm1-svc"
_OLD_DIST = "rcsa-kri-erm"
_OLD_CATALOG_ID = "Erm1"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (ERM_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-erm",
    )
