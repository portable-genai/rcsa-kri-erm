...
from rcsa_kri_erm.domain import (
    kernel,
    models,
)
...
SERVICE_NAME = "RCSA, KRI and ERM Operating Copilot"
CATALOG_ID = "rcsa-kri-erm"
REPOSITORY = "rcsa-kri-erm"
...
def _exit_control_library(container: Any) -> Any:
    return container.control_library.list_controls(TENANT)


def _exit_embeddings(container: Any) -> Any:
    return container.embeddings.embed(
        {"a": "quarterly access recertification", "b": "access recertified"}
    )


def _exit_metric_feed(container: Any) -> Any:
    return container.metric_feed.fetch(("failed_login_rate",), "2026-12-31")


def _exit_theme_feed(container: Any) -> Any:
    return container.theme_feed.themes(TENANT)
