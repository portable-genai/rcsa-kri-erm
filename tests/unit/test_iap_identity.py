...
from rcsa_kri_erm.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from rcsa_kri_erm.ports.identity import (
    VERIFIED,
...
_API_MODULE = "rcsa_kri_erm.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        # The ERM vertical's ports. The triage path this test drives never touches them, so they
        # are bound to their offline adapters across every profile: the block only has to name the
        # full declared port set (config._bindings_from refuses a partial one).
        *[
            line
            for port, cls in (
                ...
                ("theme_feed", "theme_feed:LocalThemeFeedAdapter"),
            )
            for line in (
                f"  {port}:",
                *[f"    {p}: {_PKG}.adapters.local.{cls}" for p in ("local", "gcp", "onprem")],
            )
        ],
    ]
)
