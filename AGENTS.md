...
- `src/rcsa_kri_erm/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `ERM_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `ERM_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `ERM_PROFILE` (mentioning it in a refusal message is wanted, reading
...
- **Redact before anything leaves, and by CONSTRUCTION rather than by convention.** PII is masked
  before the audit write and before any outbound payload. The raw identifier never reaches a WORM
  record or the review console. The audit half is enforced on the TYPE: `AuditEvent.__post_init__`
  masks the summary and every citation field, so none of its eight write paths can leak by
  forgetting, and a surface added later inherits the boundary. `actor` is deliberately NOT masked:
  it is the verified principal and an address by design, so a leak scan reads the CONTENT fields
  and never a whole row. `adapters/_review_payload.py` masks every field that crosses the wire,
  `case_ref`, `source_key` and each citation's locator and title included, because a field with a
  structural-looking name is still built from text this service did not author.
- **A read outside the caller's tenant is REFUSED, never answered empty.** Tenant-scoped read
  ports raise `domain/errors.py`'s `TenantAccessDeniedError`, which every surface maps to 403. An
  empty tuple is a successful answer indistinguishable from "this tenant has no controls", and the
  engines then report a verdict computed over nothing as a finding. The empty tenant is refused on
  the same rule, and no agent tool may default its `tenant`: the parameter is right, a default that
  names a partition holding data is the defect. `tests/unit/test_tenant_isolation.py` is the
  standing gate.
